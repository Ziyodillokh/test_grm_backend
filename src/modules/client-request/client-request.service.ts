...
export class ClientRequestService {
  ...
  async getAll(options: IPaginationOptions): Promise<Pagination<ClientRequest>> {
  ...
  async deleteOne(id: string) {
    const response = await this.clientRequest.softDelete(id).catch(() => {
      throw new NotFoundException('data not found');
    });
    return response;
  }
  ...
  async restoreOne(id: string) {
    const response = await this.clientRequest.restore(id).catch(() => {
      throw new NotFoundException('data not found');
    });
    return response;
  }
