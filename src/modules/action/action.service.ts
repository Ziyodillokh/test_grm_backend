      ...
      if (action.type === 'order' && kassa?.id && info?.kassa?.id === kassaId) {
        ...
        if (existDate) {
          ...
          const newOrder = this.orderRepository.create({
            ...
            date: info.kassa.createdAt,
            price: info.price,
            createdBy: action.user.id,
            plasticSum: info.plastic,
            discountSum: info.discount,
            netProfitSum: info.netProfit,
            discountPercentage: info.discountPercentage,
            additionalProfitSum: info.additionalProfit,
            status: info.status,
            seller: info.seller,
            isDebt: info.isDebt,
            kv: info.kv,
