...
export class BannerService {
  ...
  async deleteOne(id: string) {
    ...
    const response = await this.bannerRepository.softDelete({ id, index: banner.index }).catch(() => {
      throw new NotFoundException('Size not found');
    });
    return response;
  }
  ...
  async restore(id: string): Promise<void> {
    const result = await this.bannerRepository.restore(id);
    if (result.affected === 0) {
      throw new NotFoundException(`Banner with id ${id} not found or not deleted`);
    }
  }
