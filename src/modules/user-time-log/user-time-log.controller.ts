...
export class UserTimeLogController {
  ...
  async getTimeLogs(@Query() query: any) {
    // async getTimeLogs(@Query() query: any): Promise<Pagination<UserTimeLog>> {
    let where = {
      user: {
    ...
    // return await this.userTimeLogService.getAll(where, { page: +query?.page || 1, limit: +query?.limit || 20 });
    return await this.userTimeLogService.getAll(where);
  }
