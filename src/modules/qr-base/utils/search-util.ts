    ...
    ${
      ...
      : 
       `
        ...
        to_json(sh) as shape,
        to_json(st) as style,
        to_json(ct) as country,
        to_json(fc) as factory
       ` 
    }
FROM qrbase AS p
         LEFT JOIN
     model AS m ON p."modelId" = m.id
         LEFT OUTER JOIN
     collection col on col.id = p."collectionId"
         LEFT JOIN
     ...
     style AS st ON p."styleId" = st.id
         LEFT JOIN
     country AS ct ON p."countryId" = ct.id
         LEFT JOIN
     factory AS fc ON p."factoryId" = fc.id 
WHERE (SELECT COUNT(*)
       FROM (SELECT DISTINCT LOWER(word) AS word
             FROM (SELECT REGEXP_SPLIT_TO_TABLE(LOWER('%${search}%'), ' ') AS word) AS words) AS unique_words
       WHERE CONCAT_WS(' ', c.title, m.title, s.title, sh.title, st.title, p.code, col.title, m.title, ct.title, fc.title) ILIKE
